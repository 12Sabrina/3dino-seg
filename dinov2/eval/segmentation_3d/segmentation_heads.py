...
class UNETRHead(nn.Module):
    ...
    def forward_features_multi(self, x_in):
        ...
        x = x.permute(0, 2, 1).reshape(B, self.hidden_size*selfinput_channels, -1).permute(0, 2, 1)
...
class ClassificationHead(nn.Module):
    def __init__(self, feature_model, input_channels, num_classes, autocast_ctx):
        super().__init__()
        self.feature_model = feature_model
        self.input_channels = input_channels
        self.autocast_ctx = autocast_ctx
        ...
        self.channel_merge = nn.Linear(self.hidden_size * self.input_channels, self.hidden_size)
        self.act_fn = nn.GELU()
    ...
    def forward(self, x_in):
        ...
        x_reshape = x_in.reshape(-1, 1, *x_in.shape[2:])
        with self.autocast_ctx():
            # Get the CLS token from the last layer (n=23 or similar)
            # Depending on the model, get_intermediate_layers returns tokens or CLS
            outputs = self.feature_model.get_intermediate_layers(
                x_reshape,
                n=[23],
                return_class_token=True
            )
            # outputs is a list of tuples (tokens, cls_token)
            cls_token = outputs[0][1] # [B*C, hidden_size]

        # Reshape to B, C*hidden_size
        cls_token = cls_token.reshape(B, self.input_channels * self.hidden_size)
        cls_token = self.act_fn(self.channel_merge(cls_token))
