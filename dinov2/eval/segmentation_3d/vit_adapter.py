...
class ViTAdapter(nn.Module):
    def __init__(self, vit_model, input_channels, pretrain_size=112, conv_inplane=32, n_points=4,
        ...
        # Determine pretrain_size from vit_model.pos_embed
        if hasattr(self.vit_model, "pos_embed") and self.vit_model.pos_embed is not None:
            num_patches = self.vit_model.pos_embed.shape[1] - 1
            side = int(round(num_patches ** (1/3)))
            ps = self.vit_model.patch_size if isinstance(self.vit_model.patch_size, int) else self.vit_model.patch_size[0]
            self.pretrain_size = (side * ps, side * ps, side * ps)
        else:
